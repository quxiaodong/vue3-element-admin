<template>
  <div class="mb-4 p-4 rounded page-container">
    <el-form
      ref="dom"
      :model
      :rules
...
const labelWidth = computed(() => (isMobile.value ? 'auto' : ''))

const reset = () => {
  const value = initValue()
  model.value = { ...value }
  params.value = { ...value, pageSize: params.value.pageSize }

  verify().then(() => emits('reset'))
}

const search = () => {
  params.value = { ...model.value, pageSize: params.value.pageSize }
